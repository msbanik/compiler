/**************************************************************
  ...
  Modified 9/6/07 [Geoffrey Smith]
    Changed "assert" to "assrt", since "assert" is reserved in J2SE-1.4.
...
class CEmit
{
  ...
  private void set
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.assrt(null != spec);
	    CUtility.assrt(null != outstream);
	  }
  ...
  /*void emit_imports
      ...
      throws java.io.IOException      
	{
	  ...
	  if (CUtility.DEBUG)
	    {
	      CUtility.assrt(null != m_spec);
	      CUtility.assrt(null != m_outstream);
	    }*/
  ...
  private void emit_table
	...
	for (elem=0; elem<size; elem++) {
	    ...
	    CUtility.assrt(dtrans.m_dtrans.length==m_spec.m_dtrans_ncols);
  ...
  private void emit_actions 
      ...
      throws java.io.IOException
	{
	  ...
	  if (CUtility.DEBUG)
	    {
	      CUtility.assrt(m_spec.m_accept_vector.size() 
...
class CMakeNfa
{
  ...
  private void set
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.assrt(null != input);
	    CUtility.assrt(null != lexGen);
	    CUtility.assrt(null != spec);
	  }
  ...
  void allocate_BOL_EOF
	...
	{
	  CUtility.assrt(CSpec.NUM_PSEUDO==2);
...
class CMinimize
{
  ...
  private void reduce
	...
	for (reduced_ncols = 0; ; ++reduced_ncols)
	  {
		...
		for (i = 0; i < reduced_ncols; ++i)
		  {
		    CUtility.assrt(-1 != m_spec.m_col_map[i]);
		  }
	      }
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.assrt(false == set.get(i));
		CUtility.assrt(-1 == m_spec.m_col_map[i]);
	      }
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.assrt(k == reduced_ncols);
	  }
	...
	for (reduced_nrows = 0; ; ++reduced_nrows)
	  {
		...
		for (i = 0; i < reduced_nrows; ++i)
		  {
		    CUtility.assrt(-1 != m_spec.m_row_map[i]);
		  }
	      }
	...
	if (CUtility.DEBUG)
	  {
	    ...
	    CUtility.assrt(k == reduced_nrows);
	  }
      }
	...
	while (old_group_count != group_count)
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.assrt(m_group.size() == group_count);
	      }
		...
		for (j = 1; j < group_size; ++j)
		  {
			...
			if (goto_first != goto_next
				...
				|| m_ingroup[goto_next] != m_ingroup[goto_first]))
			  {
			    if (CUtility.DEBUG)
			      {
				CUtility.assrt(dtrans_group.elementAt(j) == next);
			      }
			    ...
			    if (CUtility.DEBUG)
			      {
				CUtility.assrt(m_group.contains(new_group)
						== true);
				CUtility.assrt(m_group.contains(dtrans_group)
						== true);
				CUtility.assrt(dtrans_group.contains(first)
						== true);
				CUtility.assrt(dtrans_group.contains(next)
						== false);
				CUtility.assrt(new_group.contains(first)
						== false);
				CUtility.assrt(new_group.contains(next)
						== true);
				CUtility.assrt(dtrans_group.size() == group_size);
				CUtility.assrt(i == m_ingroup[first.m_label]);
				CUtility.assrt((m_group.size() - 1) 
	...
	for (i = 0; i < size; ++i)
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.assrt(i == dtrans.m_label);
		CUtility.assrt(false == group_found);
		CUtility.assrt(group_count == m_group.size());
	      }
		...
		if (CUtility.DEBUG)
		  {
		    CUtility.assrt(false == group_found);
		    CUtility.assrt(0 < dtrans_group.size());
		  }
		...
		if (CUtility.SLOW_DEBUG)
		  {
			...
			CUtility.assrt(check.m_accept == first.m_accept);
		      }
		  }
		...
		if (first.m_accept == dtrans.m_accept)
		  {
		    ...
		    if (CUtility.DEBUG)
		      {
			CUtility.assrt(j == m_ingroup[dtrans.m_label]);
		      }
...
class CNfa2Dfa
{
	...
	for (istate = 0; nstates > istate; ++istate)
	  {
	    ...
	    */
		...
		if (CUtility.DEBUG)
		  {
		    CUtility.assrt(false == dfa.m_mark);
		  }
		...
		for (i = 0; i < m_spec.m_dtrans_ncols; ++i)
		  {
		    if (CUtility.DEBUG)
		      {
			CUtility.assrt(0 <= i);
			CUtility.assrt(m_spec.m_dtrans_ncols > i);
		      }
		    ...
		    if (CUtility.DEBUG)
		      {
			CUtility.assrt((null == bunch.m_nfa_set 
		    ...
		    if (CUtility.DEBUG)
		      {
			CUtility.assrt(nextstate < m_spec.m_dfa_states.size());
		      }
		...
		if (CUtility.DEBUG)
		  {
		    CUtility.assrt(m_spec.m_dtrans_vector.size() == dfa.m_label);
		  }
  ...
  private void e_closure
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.assrt(null != bunch);
	    CUtility.assrt(null != bunch.m_nfa_set);
	    CUtility.assrt(null != bunch.m_nfa_bit);
	  }
	...
	for (i = 0; i < size; ++i)
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.assrt(bunch.m_nfa_bit.get(state.m_label));
	      }
	...
	while (false == nfa_stack.empty())
	  {
		...
		if (CUtility.DEBUG)
		  {
		    CUtility.assrt(null != bunch.m_accept);
		    CUtility.assrt(CSpec.NONE == bunch.m_anchor
		...
		if (null != state.m_next)
		  {
			...
			if (CUtility.DEBUG)
			  {
			    CUtility.assrt(false == bunch.m_nfa_bit.get(state.m_next.m_label));
			  }
		...
		if (null != state.m_next2)
		  {
			...
			if (CUtility.DEBUG)
			  {
			    CUtility.assrt(false == bunch.m_nfa_bit.get(state.m_next2.m_label));
			  }
...
class CLexGen 
{
  ...
  void generate
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.assrt(null != this);
	    CUtility.assrt(null != m_outstream);
	    CUtility.assrt(null != m_input);
	    CUtility.assrt(null != m_tokens);
	    CUtility.assrt(null != m_spec);
	    CUtility.assrt(m_init_flag);
	  }
  ...
  private char[] packCode
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.assrt(INIT_CODE == specified 
  ...
  private void userRules
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.assrt(END_OF_INPUT == m_spec.m_current_token);
	  }
	...
	for (rep_elem = 0; rep_elem < start_macro; ++rep_elem)
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.assrt(rep_elem < replace.length);
	      }
	  }
  ...
  private void saveMacro
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.assrt(0 < count_def);
	    CUtility.assrt(0 < count_name);
	    CUtility.assrt(null != m_spec.m_macros);
	  }
  ...
  private void saveStates
	...
	if (CUtility.DEBUG)
	  {
	    CUtility.assrt('%' == m_input.m_line[0]);
	    CUtility.assrt('s' == m_input.m_line[1]);
	    CUtility.assrt(m_input.m_line_index <= m_input.m_line_read);
	    CUtility.assrt(0 <= m_input.m_line_index);
	    CUtility.assrt(0 <= m_input.m_line_read);
	  }
  ...
  private void details
	...
	while (names.hasMoreElements())
	  {
	    ...
	    if (CUtility.DEBUG)
	      {
		CUtility.assrt(null != name);
		CUtility.assrt(null != def);
	      }
	...
	if (false == m_spec.m_count_chars)
	...
	else
	  {
	    if (CUtility.DEBUG)
	      {
		CUtility.assrt(m_spec.m_count_lines);
	      }
...
final class SparseBitSet implements Cloneable {
	...
	CUtility.assrt(size<bits.length);
	insert_block(idx, bnum);
    }
    private void insert_block(int idx, int bnum) {
	CUtility.assrt(idx<=size);
	CUtility.assrt(idx==size || offs[idx]!=bnum);
	...
	CUtility.assrt(!a.get(0) && !a.get(1));
	CUtility.assrt(!a.get(123329));
	a.set(0); CUtility.assrt(a.get(0) && !a.get(1));
	a.set(1); CUtility.assrt(a.get(0) && a.get(1));
	a.clearAll();
	CUtility.assrt(!a.get(0) && !a.get(1));
	...
	for (int n=0; n<ITER; n++) {
	    ...
	    CUtility.assrt(a.get(rr) && !a.get(rr+1) && !a.get(rr-1));
	    for (int i=0; i<v.size(); i++)
		CUtility.assrt(a.get(((Integer)v.elementAt(i)).intValue()));
	}
	SparseBitSet b = (SparseBitSet) a.clone();
	CUtility.assrt(a.equals(b) && b.equals(a));
	for (int n=0; n<ITER/2; n++) {
	    ...
	    CUtility.assrt(!b.get(m));
	}
	CUtility.assrt(!a.equals(b));
	SparseBitSet c = (SparseBitSet) a.clone();
	SparseBitSet d = (SparseBitSet) a.clone();
	c.and(a);
	CUtility.assrt(c.equals(a) && a.equals(c));
	c.xor(a);
	CUtility.assrt(!c.equals(a) && c.size()==0);
	d.or(b);
	CUtility.assrt(d.equals(a) && !b.equals(d));
	d.and(b);
	CUtility.assrt(!d.equals(a) && b.equals(d));
	d.xor(a);
	CUtility.assrt(!d.equals(a) && !b.equals(d));
