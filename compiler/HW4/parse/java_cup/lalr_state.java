...
public class lalr_state {
   ...
   */
    ...
    protected boolean fix_with_precedence(
      ...
      /* Geoffrey Smith's patch: do not allow precedence to resolve a
	 shift/reduce conflict unless *both* the production and the
	 lookahead have a precedence.  This stops CUP from silently
	 resolving conflicts involving terminals that don't have a
	 declared precedence. */
      if (p.precedence_num() == assoc.no_prec ||
	  term.precedence_num() == assoc.no_prec)
        return false;
      /* End of Geoffrey Smith's patch. */
